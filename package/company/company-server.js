    ...
    if (check_auth) {
      if (req.url == "/") {
        if (req.headers?.cookie) {
          if (api_website_files_handler.CheckValidSessionCookie(req.headers.cookie)) {
            // cookie authorised
  ...
  CheckValidSessionCookie: function (cookie_header) {
    ...
    for (const _cookie of cookies) {
      ...
      if (split_cookie[0] == "session_token") {
        if (api_data_handler.CheckCookie(split_cookie[1].slice(0, split_cookie[1].length - 1))) {
          return true
        }
        else {
          return false
...
class CompanyDataHandler {
  ...
  async SendCurrentData(req, res) {
    ...
    res.end(JSON.stringify(this.config_file != undefined ? this.config_file : {
